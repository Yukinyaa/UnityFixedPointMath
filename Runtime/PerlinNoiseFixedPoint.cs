...
// Thanks to ShaunAU@Stack for the link to code: https://stackoverflow.com/questions/31718221/fixed-point-perlin-noise-returns-sum-of-input-points-as-output
...
namespace FixMath.NET
{
    public static class PerlinNoiseFixedPoint
    {
        ...
        public const int ONE = 1 << FRACTIONAL_PLACES; // 65536 = 2^16
        ...
        public static int Noise3D(int x, int y, int z)
        {
            ...
            return Lerp(w, Lerp(v, Lerp(u, Grad(p[AA], x, y, z),
                                                   Grad(p[BA], x - ONE, y, z)),
                                         Lerp(u, Grad(p[AB], x, y - ONE, z),
                                                   Grad(p[BB], x - ONE, y - ONE, z))),
                               Lerp(v, Lerp(u, Grad(p[AA + 1], x, y, z - ONE),
                                                   Grad(p[BA + 1], x - ONE, y, z - ONE)),
                                         Lerp(u, Grad(p[AB + 1], x, y - ONE, z - ONE),
                                                   Grad(p[BB + 1], x - ONE, y - ONE, z - ONE))));
        }
        ...
        {
            int t0 = fadeLUT[t >> 8];
            int t1 = fadeLUT[Math.Min(255, (t >> 8) + 1)];
            return t0 + (((t & 255) * (t1 - t0)) >> 8); // interpolates between f(t) and f(t+1)
                                                        //t0 + (((t % 256) * (t1 - t0)) / 256)
        }
        ...
        static int[] fadeLUT = new int[256] {
