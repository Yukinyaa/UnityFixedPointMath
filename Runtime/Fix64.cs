...
@"
        ...
        static int ToPerlinReprisentationInt(Fix64 n)
        {
            return (int)(n.RawValue >> (FRACTIONAL_PLACES - PerlinNoiseFixedPoint.FRACTIONAL_PLACES));
        }
        public static Fix64 Perlin(Fix64 x, Fix64 y, Fix64 z)
        {
            // 1.0 IS REPRESENTED BY 2^16
            // DECIMAL PLACES
            return Fix64.FromRaw( 
                    (long)PerlinNoiseFixedPoint.Noise3D(ToPerlinReprisentationInt(x), ToPerlinReprisentationInt(y), ToPerlinReprisentationInt(z)) << (FRACTIONAL_PLACES - PerlinNoiseFixedPoint.FRACTIONAL_PLACES)
                );
        }
    }
}
